...
// Завантажуємо категорії з вимкненим SSR
...
// Функція розумної транслітерації українського тексту в латиницю для URL
const transliterate = (text: string) => {
  ...
  return text
    ...
    .join('')
    .trim()
    .replace(/[\s_-]+/g, '-')   // Замінює пробіли та підкреслення на дефіси
    .replace(/[^\w-]/g, '')     // Видаляє спецсимволи, залишаючи тільки букви, цифри та дефіс
    .replace(/^-+|-+$/g, '')    // Прибирає дефіси на початку та в кінці рядка
}

// Слідкуємо за заголовком і автоматично генеруємо робочий Slug
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  try {
    ...
    toast.add({ title: 'Успішно', description: 'Статтю створено', color: 'success' })
    router.push('/admin/posts')
  } catch (error: any) {
    console.error(error)

    const backendMessage = error.data?.message || 'Не вдалося зберегти статтю.'
...
<template>
  <div class="p-8 max-w-2xl mx-auto">
    <div class="mb-6 flex items-center gap-4">
      ...
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Створення статті</h1>
    </div>
    ...
    <UCard>
      <UForm :schema="schema" :state="state" class="space-y-6" @submit="onSubmit">
        ...
        <UFormField label="Заголовок статті" name="title" required>
          <UInput v-model="state.title" placeholder="Введіть заголовок..." class="w-full" />
        </UFormField>

        <UFormField label="Slug (URL)" name="slug" required>
          <UInput v-model="state.slug" placeholder="url-статті" class="w-full" />
        </UFormField>
        ...
        <UFormField label="Текст статті" name="content_raw" required>
          <UTextarea v-model="state.content_raw" :rows="6" placeholder="Напишіть текст статті тут..." class="w-full" />
        </UFormField>

        <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-800">
          <UButton to="/admin/posts" color="neutral" variant="soft">Скасувати</UButton>
          <UButton type="submit" color="primary">Опублікувати статтю</UButton>
        </div>
      </UForm>
    </UCard>
  </div>
